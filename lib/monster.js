...
exports.printInfo = function (type) {
    ...
    if (type.savingThrows && type.savingThrows.length > 1) console.log(b("Saving Throws: ") + type.savingThrows);
    if (type.skills && type.skills.length > 1) console.log(b("Skills: ") + type.skills);
    if (type.damageResistances && type.damageResistances.length > 1) console.log(b("Damage Resistances: ") + type.damageResistances);
    if (type.damageVulnerabilities &&type.damageVulnerabilities.length > 1) console.log(b("Damage Vulnerabilities: ") + type.damageVulnerabilities);
    if (type.damageImmunities && type.damageImmunities.length > 1) console.log(b("Damage Immunities: ") + type.damageImmunities);
    if (type.conditionImmunities && type.conditionImmunities.length > 1) console.log(b("Condition Immunities: ") + type.conditionImmunities);
    if (type.senses && type.senses.length > 1) console.log(b("Senses: ") + type.senses);
    if (type.languages && type.languages.length > 1) console.log(b("Languages: ") + type.languages);
    //Output the list of abilities (if it has any)
    if (type.abilities && type.abilities.length > 0) {
        console.log();
...
function parseMonster(lines) {
    ...
    const attributeLines = [lines[7], lines[9], lines[11], lines[13], lines[15], lines[17]];
    console.log(attributeLines);
    m.str = lines[7].split(" ")[0];//First part of the seventh line
    m.dex = lines[9].split(" ")[0];//First part of the eight line
    m.con = lines[11].split(" ")[0];//First part of the ninth line
    m.int = lines[13].split(" ")[0];//First part of the tenth line
    m.wis = lines[15].split(" ")[0];//First part of the eleventh line
    m.chr = lines[17].split(" ")[0];//First part of the twelfth line
    //Prompt skills line
