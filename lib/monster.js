...
function parseMonster(lines) {
    ...
    //Prompt skills line
    m.savingThrows = findLine(lines, "Saving Throws").replace("Saving Throws ", "").trim();
    m.skills = findLine(lines, "Skills").replace("Skills ", "").trim();//Second part of the skills
    m.damageResistances = findLine(lines, "Damage Resistances").replace("Damage Resistances ", "").trim();
    m.damageImmunities = findLine(lines, "Damage Immunities").replace("Damage Immunities ", "").trim();
    m.damageVulnerabilities = findLine(lines, "Damage Vulnerabilities").replace("Damage Vulnerabilities ", "").trim();
    m.conditionImmunities = findLine(lines, "Condition Immunities").replace("Condition Immunities ", "").trim();
...
function findLine(lines, category) {
    ...
    for (var i = 0; i < lines.length; i++) {
        if (lines[i].indexOf(category) == 0) return lines[i];
    }
