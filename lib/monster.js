...
exports.monster = function (arguments) {
    ...
    if (args[0] === 'create') {
        ...
        if (args.length < 2) console.log("You need to specify a file to parse!");
        else parseMonster(fs.readFileSync(args[1], "utf-8").replace(/\r/g, '').split("\n"));
    }
}
...
function parseMonster(lines) {
    ...
    m.hp = lines[4].replace('Hitpoints ', '').trim();//Fifth line, replace name of category
    ...
    m.dex = lines[7].split(" ")[0];//First part of the eight line
    m.con = lines[7].split(" ")[0];//First part of the ninth line
    m.int = lines[7].split(" ")[0];//First part of the tenth line
    m.wis = lines[7].split(" ")[0];//First part of the eleventh line
    m.chr = lines[7].split(" ")[0];//First part of the twelfth line
    //Prompt skills line
    m.skills = findLine(lines, "Skills").replace("Skills ", "").trim();//Second part of the skills
    m.senses = findLine(lines, "Senses").replace("Senses ", "").trim();//2nd part of the senses
    m.languages = findLine(lines, "Languages").replace("Languages ", "").trim()//Rest of langs.
    ...
    let lastLines = lines.splice(challengeIndex - 2);
