...
exports.book = function(theme) {
    ...
    if (chance >= 7) return chalk.rgb(0, 255, 255)("[Skillbook] ") + "Seems to be instructional manual of some sort\n" + getSkillbook();
    else if (chance >= 6) return chalk.yellow("[History] ") + "Seems to be some boring academic stuff\n" + getHistory();
    else if (chance >= 5) return chalk.red("[Anatomy] ") + "Yay! Pretty pictures inside!\n" + getAnatomy();
    else if (chance >= 4) return chalk.magenta("[Arcane] ") + "Mom told me to keep away from books that look like this..\n" + getArcane();
    else if (chance >= 3) return chalk.blue("[Tales & Legends] ") + "Mom used to read me this every night..\n" + getTales();
    else if (chance >= 2) return chalk.cyan("[Story] ") + "Hey! I heard about this!\n" + getStory();
    else if (chance >= 1) return chalk.green("[Botany] ") + "What could be interesting about plants?\n" + getBotany();
}
...
function getSkillbook() {
    let chance = roll("d6");
    if (chance >= 6) return chalk.bold("Let's play dead:") + " This 100 or so page volume consists of a childlike representation with several brightly coloured pictures of various recipes to prepare the dead for undeath.";
    else if (chance >= 5) return chalk.bold("Taste of victory:") + " The book details a fencing techniques, with focus on various dirty tricks you can use to win a duel and get away with it.";
    else if (chance >= 4) return chalk.bold("Shock and Awe:") + " The book contains 20 basic campfire recipes, which are described in great detail and accompanied by multitude of illustrations. Anybody can use this book to craft one of the 20 meals, all of which make use of venision or other meat.";
    else if (chance >= 3) return chalk.bold("Smuggler’s teachings:") + " The collected teachings of a dozen master blacksmiths, armorers, and artificers, walks the reader step by step through the art of metalwork, from the most basic of tools to the creation of magic weapons and armor. The text is accompanied by dozens of intricatly detailed plates showing tools and techniques.";
    else if (chance >= 2) return chalk.bold("Liber ex Vasis:") + " A rather thin volume about the comestible plant life found in the Underdark and the different ways to prepare them. The author begins by explaining his firsthand experience tasting and testing all the recipes and flora available to the underdark. After detailing a few recipes, the writings become more rambly and saccaded. A certain plant begins to come up in several of the recipes towards the end of the book. The final page is just a repetition of that same plant's name over and over again until the words just trail off the page.";
    else if (chance >= 1) return chalk.bold("A Herbalist's Guide to Surviving:") + " This diary details the accounts a famous halfling smuggler, who was best known for smuggling his home made rum into kingdoms throughout the world during the great alcohol depression. With proper study the reader can reproduce the famous Sweetfoot Rum recipe from the different mentions, hints and references scattered across the pages of this book.";
}
...
function getArcane() {
    let chance = roll("d12");
    if (chance >= 12) return chalk.bold("The Forgotten Art:") + " This massive tome is a fine source of information about all things undead, and was written by one Lord Zeiram, who later ascended to lichhood. It has long been rumored that it is a good starting point for a would-be lich, and it has thus been banned in many a juristiction, but truth be told it is of little use when it comes to becoming a lich, unless one counts the potential benefit of more effective minions.";
    else if (chance >= 11) return chalk.bold("From Beyond the Veil:") + " This introductory tome is required reading at many a magical academy, but is of relatively little value in regards to the truly esoteric. Introduces the various elements and elementals, and discusses the purpose/traits of the various elemental planes and the elemental chaos.";
    else if (chance >= 10) return chalk.bold("Edicts of Incantations:") + " This book is a standalone work from eccentric elven author Marybeth Hight, a scholar of the Feywild and its denizens. It was originally intended as a primer for would-be plane shifters and astral travellers, but the manuscript was partially destroyed in a fire and now the contents are damaged. The current owner has gone mildly insane trying to piece the manuscript back together, and has scrawled almost two year's worth of ramblings and half-thoughts in the margins of the text. While the work no longer primes the reader for the act of travelling to the Feywild, the combination of reading materials contained within the folio binding now prep the reader's mind for the bewildering and maddening magics encountered there (giving advantage against the enchantment and illusion effects created by denizens of the Feywild)";
    else if (chance >= 9) return chalk.bold("Arcane Secrets:") + " A generous manuscript contains description of inner workings of various contraptions and automatons and instruction manual to programing automatons, allowing any intermediate transmuter to create their very own contraptions!";
    else if (chance >= 8) return chalk.bold("Sinister Discoveries:") + " Reading this little book takes about an hour. It contains complete instructions how to cast two cantrips from the wizard list of spells, allowing the reader to cast them for the remainder of the day.";
    else if (chance >= 7) return chalk.bold("Theatrical Uses of Illusion:") + " A dry but informative text detailing the blending of Positive Energy and Negative Energy (which he refers to as “the Holy Antipodes”) to better access healing and harming magics. It’s an insightful work for healers and necromancers alike, but those who read carefully and follow Sahl’s train of logic may unlock a new path of power (read gain access to a prestige class).";
    else if (chance >= 6) return chalk.bold("Antipodean Harmonies: Written by the mage playwright Rodger Goldhammer, this semi autobiography shows how the famed thespian worked intricate illusion spells into his plays. Chapters included \"Combining natural and magical light\", \"Canned vs conjured thunder:") + " a discussion\", and \"Loss of concentration, or why it is vital to wear undergarments beneath an illusionary costume\".";
    else if (chance >= 5) return chalk.bold("A children's guide to necromancy:") + " This book contains procedures regarding conjuring and exorcising acient horrors of the deep seas.";
    else if (chance >= 4) return chalk.bold("Skritzlbon's Contraptions:") + " Rather well known and a piece of every wizard’s library, this book contains instructions on divination basics, and tips how to not anger the customers with unfortunate events to come.";
    else if (chance >= 3) return chalk.bold("Liminal Zones and You:") + " When opened, this book creates an spectral dog that starts reproducing the book contents with the skill of experienced rhetorician. Given it's a dog, it's speech consists of different barks, whines and howls. The actual contets are studies regarding blink dogs. Why this book was named the way it was is a mystery not even the author knows answer to.";
    else if (chance >= 2) return chalk.bold("Elements and Elementals:") + " A collection of stories and essays focusing on a first-hand account of the journey from life to death told to the author by a ghost, a resurrected individual or a soul called forth from the other realms. Critics suggest she sensationalizes the tales a bit to make death sound more frightening than it is, to which she challenges them to explore death on their own and then come back to talk about it.";
    else if (chance >= 1) return chalk.bold("From Ghouls to Ghosts:") + " The book is chaotic and hard to understand, written in an unkown dialect which makes it very slow to read. Contained within are the studies of a powerful necromancer on how interplanar travel might be used to achieve functional immortality.";
}
...
function getTales() {
    let chance = roll("d10");
    if (chance >= 10) return chalk.bold("A Sheep's Tail: A propagandic tale featuring two main characters:") + " A delusional young man who joins an order of paladins and sets about “restoring good and order to the world” and a young necromancer who only wants to help people. The story ends with the paladin murdering the necromancer, as the necromancer had spent his magical powers healing some innocent villagers who were hurt in a goblin raid. It has been theorised that the story might have been written and distrubuted by an ancient vampire wizard as part of his smear campaign against so called \"good\" organised religions and militant orders, but this has never been conclusively proven.";
    else if (chance >= 9) return chalk.bold("Tale of Cons and Scams:") + " The first installment of Critter and his friends' adventures! Watch as they journey into the Underdark searching for some bountiful booty to bring back home. Critter and his friends soon discover that the denizens of the Underdark don't take kindly to guests. Read carefully as duergars, and drow pin Critter's friends' still live bodies to crosses while torturing their mutilated bodies for being the filthy thieves that they are! A journey of friendship, kindness, dark and unending wallows of despair, and bravery that's fun for the whole family!";
    else if (chance >= 8) return chalk.bold("A Night to Remember:") + " Follow Critter and his new friends in their wacky adventures across the Forgotten Realms! They've been up to some crazy hijinxes together ever since they first met. In this latest installment, watch as Critter and his friends are pursued by the demons of the Nine Hells for betraying a pact with Lolth. Critter gets to watch in stunned horror as each of his friends is burnt to a crisp by a demonic army! Read carefully as they discover the value of friendship, kindness, fire, and bravery!";
    else if (chance >= 7) return chalk.bold("The Stonemason's Son:") + " A young kobold falls for her laconic draconic master in this classic tale of love, taboo, discovery, and betrayal.";
    else if (chance >= 6) return chalk.bold("The Unaligned Monk:") + " A story in the form of a collection of letters from a dwarf stonemason's son that left the trade to be an adventurer.";
    else if (chance >= 5) return chalk.bold("The Story of Graye:") + " The story of a slave forced to be a pit-fighter who turned to meditation as an escape from his violent life. This book is not well written, and is probably an earlier work of a novice author that never reached wide spread fame.";
    else if (chance >= 4) return chalk.bold("The Creepy Crawly Cremation Story:") + " A short story that describes a case where the shadow realm spills into the material plane and haunts the small town of Graye.";
    else if (chance >= 3) return chalk.bold("The Cryptic Crystalline Crucifixion Story:") + " A book filled with full-page illustrations with small captions (in an esoteric language) that tell the story of a man who seeks to fight criminals by dressing up and scaring them. If the reader doesn't know the language, it appears to be about a vampire.";
    else if (chance >= 2) return chalk.bold("Most Holy of Knights:") + " Written like a child's book with colorful pictures. Details a murderous sheep that is burned after slaying a whole town. Then reborn as an evil sheep spirit.";
    else if (chance >= 1) return chalk.bold("Travellers Musings:") + " Within is what seems to be a collection of short stories. To anyone that can read the thieves cant, the book is a guide on how to gain membership to the thieves guilds.";
}
