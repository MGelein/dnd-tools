 ...
 */
/**
 * Expands the given string untill it is the length of the provided 
 * length number. By default it adds spaces
 * @param {String} string 
 * @param {Number} length 
 * @param {String} expandor
 */
function expand(string, length, expandor){
...
const ROWS_ON_LINE = 3;
  ...
  * @param {Array} header the fiedls the object are supposed to have
  * @param {Boolean} printHeader if the header should be printed above the table
  */
 exports.table = function(values, header, printHeader){
    ...
    values.forEach(function(value){
        ...
        //Print a single line
        header.forEach(function(headerVal){
            cells.push(expand(value[headerVal], maxLengths[headerVal]));
        });
 ...
 exports.header = function(string){
    let divider = chalk.gray(expand("=", string.length + 3, "="));
