    ...
    while (running) {
        ...
        if (keyIn === 'q') {
            ...
            if (targets.length > 0) {
             ...
             //Change initiative for any actor, be it a monster or PC
             cls();
             console.log(header("CHANGE INITIATIVE"));
             //Create list of possible targets
             let targets = getNameList(true);
             //Now ask for the target
             let index = readline.keyInSelect(targets, "Who do you want to change initiative for? ");
             if (index != -1) {//Which means we actually selected someone
                 //Ask for new initiative
                 let initiative = readline.questionInt("What is the new initiative of "
                     + targets[index]+ ": ");
                 getActorByName(targets[index]).initiative = initiative;
                 sortInitiative();
             }
